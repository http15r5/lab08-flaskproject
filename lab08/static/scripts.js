// Universal scripts for all templates (base, admin, client, api_demo, feedback, order_details)
document.addEventListener('DOMContentLoaded', () => {
  ...
  // showMessage: uses #message if present, otherwise floating flash
  ...
  // Auto-hide server-rendered flash messages
  setTimeout(() => {
  ...
  function setupToggle(btnId, sectionId, iconId) {
    ...
    btn.addEventListener('click', () => {
      ...
      // maintain accessible maxHeight if used
      try {
  ...
  // register known variations found in templates:
  // admin uses: toggleProducts / sectionProducts / iconProducts
  ...
  // order_details uses: toggleProducts / productsSection / toggleIcon (and toggleIcon id)
  setupToggle('toggleProducts', 'productsSection', 'toggleIcon');
  // order_details also uses 'toggleIcon' naming; also support 'productsSection' with 'iconProducts'
  setupToggle('toggleProducts', 'productsSection', 'iconProducts');
  setupToggle('toggleProducts', 'sectionProducts', 'toggleIcon');

  // --------------------- API helpers (api_demo) ---------------------
  ...
  // FEEDBACK list + add + delete (supports both messageText and message names)
  ...
  // --------------------- Client modals (details/cancel/delete) ---------------------
  ...
  // feedback: templates use either messageText or message; handle both
  safe('feedbackForm', form => {
    form.addEventListener('submit', async (e) => {
      ...
      // if feedback form uses action with endpoint, we will post FormData; otherwise use API
      const action = form.getAttribute('action') || form.action || '';
      if (action && action.trim()) {
        // submit as form (server-side endpoint)
        try {
